(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const a of o.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&n(a)}).observe(document,{childList:!0,subtree:!0});function l(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(s){if(s.ep)return;s.ep=!0;const o=l(s);fetch(s.href,o)}})();const k=10935,m=5,u=[{name:"Sunlight",sci:"Epipelagic",from:0,to:200,light:"Full sunlight"},{name:"Twilight",sci:"Mesopelagic",from:200,to:1e3,light:"Dim blue light"},{name:"Midnight",sci:"Bathypelagic",from:1e3,to:4e3,light:"Bioluminescence only"},{name:"Abyssal",sci:"Abyssopelagic",from:4e3,to:6e3,light:"Total darkness"},{name:"Hadal",sci:"Hadalpelagic",from:6e3,to:11e3,light:"Total darkness"}];function C(e){return u.find(t=>e<t.to)??u[u.length-1]}const p=[[0,56,172,216],[80,24,126,184],[200,10,82,142],[450,9,50,104],[700,9,33,76],[1e3,7,18,50],[1600,5,11,34],[2500,4,7,24],[4e3,2,4,14],[6e3,1,2,8],[8500,0,1,4],[11e3,0,0,1]];function E(e){const t=Math.max(0,Math.min(k,e));let l=1;for(;l<p.length-1&&p[l][0]<t;)l++;const[n,s,o,a]=p[l-1],[G,A,I,F]=p[l],P=G===n?0:(t-n)/(G-n),y=(S,H)=>Math.round(S+(H-S)*P);return`rgb(${y(s,A)}, ${y(o,I)}, ${y(a,F)})`}const O=Math.log(100)/200;function $(e){return Math.exp(-O*Math.max(0,e))}function D(e){const t=$(e)*100;return e>=1e3?"0%":t>=1?`${Math.round(t)}%`:t>=.01?`${t.toFixed(2)}%`:t>=1e-4?`${t.toFixed(4)}%`:"<0.0001%"}function N(e){return Math.round(1+e/10)}const j=[{id:"flying-fish",name:"Flying Fish",sci:"Exocoetidae",depth:6,range:"Surface",size:120,side:"right",fact:"To escape tuna and marlin, it rockets out of the water and glides on outstretched fins — some stay airborne for over 200 metres before splashing back down.",svg:`
      ...
      </svg>`},{id:"clownfish",name:"Clownfish",sci:"Amphiprion ocellaris",depth:22,range:"1–15 m",size:84,side:"left",fact:"Lives inside the stinging arms of sea anemones — a coat of mucus keeps it from getting stung. Every clownfish is born male; the dominant one becomes female.",svg:`
      ...
      </svg>`},{id:"giant-manta",name:"Giant Manta Ray",sci:"Mobula birostris",depth:115,range:"Surface–120 m",size:230,side:"left",fact:"The largest ray on Earth, spanning up to 7 metres wingtip to wingtip. It filters plankton from the water and has the biggest brain of any fish — curious enough to seem to interact with divers.",svg:`
      ...
      </svg>`},{id:"giant-squid",name:"Giant Squid",sci:"Architeuthis dux",depth:920,range:"300–1,000 m",size:250,side:"left",fact:"One of the largest animals without a backbone, reaching 12–13 metres including its feeding tentacles. Its eyes are the size of dinner plates — the biggest in the animal kingdom — built to catch the faintest glow in the dark.",svg:`
      ...
      </svg>`},{id:"gulper-eel",name:"Gulper Eel",sci:"Eurypharynx pelecanoides",depth:3050,range:"500–3,000 m",size:180,side:"right",fact:"Almost all mouth — its enormous, loosely hinged jaw can open like a pelican’s pouch to swallow prey larger than itself. The rest of its body tapers into a whip-thin tail tipped with a glowing pink light.",svg:`
      ...
      </svg>`},{id:"dumbo-octopus",name:"Dumbo Octopus",sci:"Grimpoteuthis",depth:3600,range:"1,000–7,000 m",size:130,side:"left",fact:"The deepest-living octopus known, named for the ear-like fins it flaps to swim. Instead of squirting ink — useless in the dark — it simply flushes red or pale to signal, and swallows its prey whole.",svg:`
      ...
      </svg>`},{id:"giant-isopod",name:"Giant Isopod",sci:"Bathynomus giganteus",depth:4500,range:"170–2,140 m+",size:120,side:"right",fact:"A deep-sea relative of the woodlouse in your garden, but grown to the size of a house cat. It scavenges whatever sinks from above and can survive years between meals — one in captivity refused food for over five years.",svg:`
      ...
      </svg>`},{id:"sea-pig",name:"Sea Pig",sci:"Scotoplanes globosa",depth:5100,range:"1,000–6,000 m",size:110,side:"left",fact:"A plump, pinkish sea cucumber that walks the abyssal mud on stubby water-filled legs, hoovering up the richest sediment like a vacuum. Herds of them gather wherever food falls, sometimes hundreds all facing the same way.",svg:`
      ...
      </svg>`},{id:"tripod-fish",name:"Tripod Fish",sci:"Bathypterois grallator",depth:5700,range:"900–4,700 m",size:130,side:"right",fact:"Perches motionless on three long, stiffened fin rays like a camera on a tripod, facing into the current. Nearly blind, it waits with its front fins spread wide to feel for tiny prey drifting past in the dark.",svg:`
      ...
      </svg>`},{id:"mariana-snailfish",name:"Mariana Snailfish",sci:"Pseudoliparis swirei",depth:8e3,range:"6,900–8,000 m",size:120,side:"left",fact:"The deepest fish ever filmed, thriving where the pressure would crush a submarine. Its body has almost no scales and a soft, translucent, tadpole-like shape — a special molecule in its cells keeps its proteins from being squeezed apart.",svg:`
      ...
      </svg>`},{id:"giant-amphipod",name:"Giant Amphipod",sci:"Hirondellea gigas",depth:9500,range:"6,000–11,000 m",size:100,side:"right",fact:"A shrimp-like scavenger that swarms the very bottom of the Mariana Trench, feasting on anything that falls. It even eats sunken wood, using gut enzymes to digest it — and armours its shell with aluminium pulled from the seawater.",svg:`
      ...
      </svg>`}],d=document.getElementById("ocean"),g=document.documentElement,c={depth:document.getElementById("hud-depth"),zone:document.getElementById("hud-zone"),zoneSci:document.getElementById("hud-zone-sci"),light:document.getElementById("hud-light"),pressure:document.getElementById("hud-pressure")},x=k*m;d.style.height=`calc(${x}px + 100svh)`;function M(e){return`calc(${Math.round(e*m)}px + 50svh)`}const h=document.createElement("div");h.id="ruler";h.setAttribute("aria-hidden","true");h.style.top="50svh";h.style.height=`${x}px`;d.appendChild(h);for(let e=500;e<=k;e+=500){const t=document.createElement("span");t.className="ruler-label",t.textContent=`${e.toLocaleString("en-US")} m`,t.style.top=M(e),d.appendChild(t)}for(const e of u.slice(1)){const t=document.createElement("div");t.className="zone-marker",t.style.top=M(e.from),t.innerHTML=`
    <div class="zone-marker-text">
      ${e.name} Zone · ${e.from.toLocaleString("en-US")} m
      <small>${e.sci}</small>
    </div>`,d.appendChild(t)}const q=new IntersectionObserver(e=>{for(const t of e)t.isIntersecting&&(t.target.classList.add("in-view"),q.unobserve(t.target))},{rootMargin:"0px 0px -12% 0px"});for(const e of j){const t=document.createElement("article");t.className=e.depth>=1e3?"creature deep":"creature",t.dataset.side=e.side,t.dataset.id=e.id,t.style.top=M(e.depth),t.innerHTML=`
    ...
    </div>`,t.querySelector(".creature-tap").addEventListener("click",()=>R(e)),d.appendChild(t),q.observe(t)}const i=document.createElement("div");i.id="detail";i.hidden=!0;i.innerHTML=`
  ...
  </div>`;document.body.appendChild(i);const r={sprite:i.querySelector(".detail-sprite"),zone:i.querySelector("#detail-zone"),zonefull:i.querySelector("#detail-zonefull"),name:i.querySelector("#detail-name"),sci:i.querySelector("#detail-sci"),range:i.querySelector("#detail-range"),fact:i.querySelector("#detail-fact"),card:i.querySelector(".detail-card")};let b=null;function R(e){const t=C(e.depth);r.sprite.innerHTML=e.svg,r.sprite.style.maxWidth=`${Math.min(e.size*1.7,260)}px`,r.zone.textContent=`${t.name} Zone`,r.zonefull.textContent=`${t.name} · ${t.sci}`,r.name.textContent=e.name,r.sci.textContent=e.sci,r.range.textContent=e.range,r.fact.textContent=e.fact,i.dataset.zone=t.name.toLowerCase(),b=document.activeElement,i.hidden=!1,document.body.style.overflow="hidden",requestAnimationFrame(()=>i.classList.add("open")),i.querySelector(".detail-close").focus()}function L(){i.classList.remove("open"),document.body.style.overflow="";const e=()=>{i.hidden=!0,i.removeEventListener("transitionend",e)};i.addEventListener("transitionend",e),b&&b.focus()}i.addEventListener("click",e=>{e.target.hasAttribute("data-close")&&L()});document.addEventListener("keydown",e=>{e.key==="Escape"&&!i.hidden&&L()});let f=null;r.card.addEventListener("touchstart",e=>{f=e.touches[0].clientY},{passive:!0});r.card.addEventListener("touchmove",e=>{if(f===null)return;const t=e.touches[0].clientY-f;t>0&&(r.card.style.transform=`translateY(${Math.min(t,200)}px)`)},{passive:!0});r.card.addEventListener("touchend",e=>{const t=e.changedTouches[0].clientY-(f??0);r.card.style.transform="",t>90&&L(),f=null},{passive:!0});const Z=document.createElement("div");Z.id="floor";Z.innerHTML=`
  <div class="floor-line">Challenger Deep · bottom of the Mariana Trench</div>
  <span class="floor-depth">10,935 m</span>`;d.appendChild(Z);const w=document.getElementById("zone-toast"),W=w.querySelector(".zt-title"),Y=w.querySelector(".zt-sub");let T=null;function U(e){W.textContent=`The ${e.name} Zone`,Y.textContent=`${e.sci} · ${e.light}`,w.classList.add("show"),clearTimeout(T),T=setTimeout(()=>w.classList.remove("show"),2400)}const X=[{el:document.querySelector(".biolum-a"),speed:.05,tile:360},{el:document.querySelector(".biolum-b"),speed:.11,tile:440},{el:document.querySelector(".biolum-c"),speed:.19,tile:600}];let v=!1,Q=null,B=-1;function z(){v=!1;const e=Math.max(0,Math.min(window.scrollY,x)),t=e/m,l=window.innerHeight/m;g.style.setProperty("--sea-hi",E(t-l/2)),g.style.setProperty("--sea-lo",E(t+l/2)),g.style.setProperty("--sunlight",$(t).toFixed(4));const n=Math.max(0,Math.min(1,(t-900)/400));if(g.style.setProperty("--biolum",n.toFixed(3)),n>0)for(const a of X)a.el.style.transform=`translate3d(0, ${-(e*a.speed%a.tile)}px, 0)`;const s=Math.round(t);s!==B&&(B=s,c.depth.textContent=s.toLocaleString("en-US"),c.light.textContent=D(t),c.pressure.textContent=N(t).toLocaleString("en-US"));const o=C(t);if(o!==Q){const a=Q===null;Q=o,c.zone.textContent=o.name,c.zoneSci.textContent=o.sci,a||U(o)}}window.addEventListener("scroll",()=>{v||(v=!0,requestAnimationFrame(z))},{passive:!0});window.addEventListener("resize",z,{passive:!0});z();"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("./sw.js")});
