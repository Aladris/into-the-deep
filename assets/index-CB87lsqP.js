(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))r(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const c of a.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&r(c)}).observe(document,{childList:!0,subtree:!0});function o(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function r(i){if(i.ep)return;i.ep=!0;const a=o(i);fetch(i.href,a)}})();const M=10935,p=5,Q=[{name:"Sunlight",sci:"Epipelagic",from:0,to:200,light:"Full sunlight"},{name:"Twilight",sci:"Mesopelagic",from:200,to:1e3,light:"Dim blue light"},{name:"Midnight",sci:"Bathypelagic",from:1e3,to:4e3,light:"Bioluminescence only"},{name:"Abyssal",sci:"Abyssopelagic",from:4e3,to:6e3,light:"Total darkness"},{name:"Hadal",sci:"Hadalpelagic",from:6e3,to:11e3,light:"Total darkness"}];function L(e){return Q.find(t=>e<t.to)??Q[Q.length-1]}const m=[[0,56,172,216],[80,24,126,184],[200,10,82,142],[450,9,50,104],[700,9,33,76],[1e3,7,18,50],[1600,5,11,34],[2500,4,7,24],[4e3,2,4,14],[6e3,1,2,8],[8500,0,1,4],[11e3,0,0,1]];function T(e){const t=Math.max(0,Math.min(M,e));let o=1;for(;o<m.length-1&&m[o][0]<t;)o++;const[r,i,a,c]=m[o-1],[C,j,O,N]=m[o],W=C===r?0:(t-r)/(C-r),k=(A,R)=>Math.round(A+(R-A)*W);return`rgb(${k(i,j)}, ${k(a,O)}, ${k(c,N)})`}const J=Math.log(100)/200;function H(e){return Math.exp(-J*Math.max(0,e))}function U(e){const t=H(e)*100;return e>=1e3?"0%":t>=1?`${Math.round(t)}%`:t>=.01?`${t.toFixed(2)}%`:t>=1e-4?`${t.toFixed(4)}%`:"<0.0001%"}function V(e){return Math.round(1+e/10)}const D=[{id:"flying-fish",name:"Flying Fish",sci:"Exocoetidae",depth:6,range:"Surface",size:120,side:"right",fact:"To escape tuna and marlin, it rockets out of the water and glides on outstretched fins — some stay airborne for over 200 metres before splashing back down.",svg:`
      ...
      </svg>`},{id:"moon-jelly",name:"Moon Jelly",sci:"Aurelia aurita",depth:70,range:"0–1,000 m",size:118,side:"left",fact:"Drifts on the currents with almost no ability to swim against them. Its body is about 95% water, and the four pale horseshoes glowing through the bell are its reproductive organs.",svg:`
      ...
      </svg>`},{id:"bristlemouth",name:"Bristlemouth",sci:"Cyclothone",depth:760,range:"300–2,000 m",size:88,side:"left",fact:"Likely the most numerous vertebrate on Earth — there may be quadrillions of them. Barely finger-length, with a mouth full of bristle-like teeth and rows of tiny glowing dots along its body.",svg:`
      ...
      </svg>`}],n=document.getElementById("ocean"),u=document.documentElement,h={depth:document.getElementById("hud-depth"),zone:document.getElementById("hud-zone"),zoneSci:document.getElementById("hud-zone-sci"),light:document.getElementById("hud-light"),pressure:document.getElementById("hud-pressure")},Z=M*p;n.style.height=`calc(${Z}px + 100svh)`;function G(e){return`calc(${Math.round(e*p)}px + 50svh)`}const w=document.createElement("div");w.id="ruler";w.setAttribute("aria-hidden","true");w.style.top="50svh";w.style.height=`${Z}px`;n.appendChild(w);for(let e=500;e<=M;e+=500){const t=document.createElement("span");t.className="ruler-label",t.textContent=`${e.toLocaleString("en-US")} m`,t.style.top=G(e),n.appendChild(t)}for(const e of Q.slice(1)){const t=document.createElement("div");t.className="zone-marker",t.style.top=G(e.from),t.innerHTML=`
    <div class="zone-marker-text">
      ${e.name} Zone · ${e.from.toLocaleString("en-US")} m
      <small>${e.sci}</small>
    </div>`,n.appendChild(t)}const P=new IntersectionObserver(e=>{for(const t of e)t.isIntersecting&&(t.target.classList.add("in-view"),P.unobserve(t.target))},{rootMargin:"0px 0px -12% 0px"});for(const e of D){const t=document.createElement("article");t.className=e.depth>=1e3?"creature deep":"creature",t.dataset.side=e.side,t.dataset.id=e.id,t.style.top=G(e.depth),t.innerHTML=`
    ...
    </div>`,t.querySelector(".creature-tap").addEventListener("click",()=>Y(e)),n.appendChild(t),P.observe(t)}const s=document.createElement("div");s.id="detail";s.hidden=!0;s.innerHTML=`
  ...
  </div>`;document.body.appendChild(s);const l={sprite:s.querySelector(".detail-sprite"),zone:s.querySelector("#detail-zone"),zonefull:s.querySelector("#detail-zonefull"),name:s.querySelector("#detail-name"),sci:s.querySelector("#detail-sci"),range:s.querySelector("#detail-range"),fact:s.querySelector("#detail-fact"),card:s.querySelector(".detail-card")};let v=null;function Y(e){const t=L(e.depth);l.sprite.innerHTML=e.svg,l.sprite.style.maxWidth=`${Math.min(e.size*1.7,260)}px`,l.zone.textContent=`${t.name} Zone`,l.zonefull.textContent=`${t.name} · ${t.sci}`,l.name.textContent=e.name,l.sci.textContent=e.sci,l.range.textContent=e.range,l.fact.textContent=e.fact,s.dataset.zone=t.name.toLowerCase(),v=document.activeElement,s.hidden=!1,document.body.style.overflow="hidden",requestAnimationFrame(()=>s.classList.add("open")),s.querySelector(".detail-close").focus()}function z(){s.classList.remove("open"),document.body.style.overflow="";const e=()=>{s.hidden=!0,s.removeEventListener("transitionend",e)};s.addEventListener("transitionend",e),v&&v.focus()}s.addEventListener("click",e=>{e.target.hasAttribute("data-close")&&z()});document.addEventListener("keydown",e=>{e.key==="Escape"&&!s.hidden&&z()});let g=null;l.card.addEventListener("touchstart",e=>{g=e.touches[0].clientY},{passive:!0});l.card.addEventListener("touchmove",e=>{if(g===null)return;const t=e.touches[0].clientY-g;t>0&&(l.card.style.transform=`translateY(${Math.min(t,200)}px)`)},{passive:!0});l.card.addEventListener("touchend",e=>{const t=e.changedTouches[0].clientY-(g??0);l.card.style.transform="",t>90&&z(),g=null},{passive:!0});const f=document.createElement("button");f.id="divemap-toggle";f.type="button";f.setAttribute("aria-label","Open the dive line to jump to a creature");f.innerHTML=`
  ...
  <span>DIVE<br/>LINE</span>`;document.body.appendChild(f);const d=document.createElement("div");d.id="divemap";d.hidden=!0;d.innerHTML=`
  ...
  </aside>`;document.body.appendChild(d);const q=d.querySelector(".divemap-track");let I=null;for(const e of D){const t=L(e.depth);if(t.name!==I){I=t.name;const r=document.createElement("div");r.className="divemap-zone",r.dataset.zone=t.name.toLowerCase();const i=t.to>=11e3?"10,935":t.to.toLocaleString("en-US");r.innerHTML=`<span>${t.name} · ${t.from.toLocaleString("en-US")}–${i} m</span>`,q.appendChild(r)}const o=document.createElement("button");o.type="button",o.className="divemap-item",o.dataset.side=e.side,o.setAttribute("aria-label",`Jump to ${e.name} at ${Math.round(e.depth)} metres`),o.innerHTML=`
    <span class="dm-chip">
      <span class="dm-node">${e.svg}</span>
      <span class="dm-label"><b>${e.name}</b><i>${Math.round(e.depth).toLocaleString("en-US")} m</i></span>
    </span>`,o.addEventListener("click",()=>_(e)),q.appendChild(o)}function X(){d.hidden=!1,document.body.style.overflow="hidden",requestAnimationFrame(()=>d.classList.add("open"))}function S(){d.classList.remove("open"),document.body.style.overflow="";const e=()=>{d.hidden=!0,d.removeEventListener("transitionend",e)};d.addEventListener("transitionend",e)}function _(e){S();const t=window.matchMedia("(prefers-reduced-motion: reduce)").matches;window.scrollTo({top:Math.round(e.depth*p),behavior:t?"auto":"smooth"})}f.addEventListener("click",X);d.addEventListener("click",e=>{e.target.hasAttribute("data-close")&&S()});document.addEventListener("keydown",e=>{e.key==="Escape"&&!d.hidden&&S()});const B=document.createElement("div");B.id="floor";B.innerHTML=`
  <div class="floor-line">Challenger Deep · bottom of the Mariana Trench</div>
  <span class="floor-depth">10,935 m</span>`;n.appendChild(B);const y=document.getElementById("zone-toast"),K=y.querySelector(".zt-title"),ee=y.querySelector(".zt-sub");let $=null;function te(e){K.textContent=`The ${e.name} Zone`,ee.textContent=`${e.sci} · ${e.light}`,y.classList.add("show"),clearTimeout($),$=setTimeout(()=>y.classList.remove("show"),2400)}const ie=[{el:document.querySelector(".biolum-a"),speed:.05,tile:360},{el:document.querySelector(".biolum-b"),speed:.11,tile:440},{el:document.querySelector(".biolum-c"),speed:.19,tile:600}];let x=!1,b=null,F=-1;function E(){x=!1;const e=Math.max(0,Math.min(window.scrollY,Z)),t=e/p,o=window.innerHeight/p;u.style.setProperty("--sea-hi",T(t-o/2)),u.style.setProperty("--sea-lo",T(t+o/2)),u.style.setProperty("--sunlight",H(t).toFixed(4));const r=Math.max(0,Math.min(1,(t-900)/400));if(u.style.setProperty("--biolum",r.toFixed(3)),r>0)for(const c of ie)c.el.style.transform=`translate3d(0, ${-(e*c.speed%c.tile)}px, 0)`;const i=Math.round(t);i!==F&&(F=i,h.depth.textContent=i.toLocaleString("en-US"),h.light.textContent=U(t),h.pressure.textContent=V(t).toLocaleString("en-US"));const a=L(t);if(a!==b){const c=b===null;b=a,h.zone.textContent=a.name,h.zoneSci.textContent=a.sci,c||te(a)}}window.addEventListener("scroll",()=>{x||(x=!0,requestAnimationFrame(E))},{passive:!0});window.addEventListener("resize",E,{passive:!0});E();"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("./sw.js")});
