(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))a(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const r of o.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function i(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(s){if(s.ep)return;s.ep=!0;const o=i(s);fetch(s.href,o)}})();const F=10935,z=6,G=[{name:"Sunlight",sci:"Epipelagic",from:0,to:200,light:"Full sunlight"},{name:"Twilight",sci:"Mesopelagic",from:200,to:1e3,light:"Dim blue light"},{name:"Midnight",sci:"Bathypelagic",from:1e3,to:4e3,light:"Bioluminescence only"},{name:"Abyssal",sci:"Abyssopelagic",from:4e3,to:6e3,light:"Total darkness"},{name:"Hadal",sci:"Hadalpelagic",from:6e3,to:11e3,light:"Total darkness"}];function T(e){return G.find(t=>e<t.to)??G[G.length-1]}const q=[[0,56,172,216],[80,24,126,184],[200,10,82,142],[450,9,50,104],[700,9,33,76],[1e3,7,18,50],[1600,5,11,34],[2500,4,7,24],[4e3,2,4,14],[6e3,1,2,8],[8500,0,1,4],[11e3,0,0,1]];function _(e){const t=Math.max(0,Math.min(F,e));let i=1;for(;i<q.length-1&&q[i][0]<t;)i++;const[a,s,o,r]=q[i-1],[n,d,p,g]=q[i],x=n===a?0:(t-a)/(n-a),w=(M,I)=>Math.round(M+(I-M)*x);return`rgb(${w(s,d)}, ${w(o,p)}, ${w(r,g)})`}const he=Math.log(100)/200;function oe(e){return Math.exp(-he*Math.max(0,e))}function fe(e){const t=oe(e)*100;return e>=1e3?"0%":t>=1?`${Math.round(t)}%`:t>=.01?`${t.toFixed(2)}%`:t>=1e-4?`${t.toFixed(4)}%`:"<0.0001%"}function pe(e){return Math.round(1+e/10)}const y=[{id:"flying-fish",name:"Flying Fish",sci:"Exocoetidae",depth:6,range:"Surface",size:120,side:"right",fact:"To escape tuna and marlin, it rockets out of the water and glides on outstretched fins — some stay airborne for over 200 metres before splashing back down.",svg:`
      ...
      </svg>`},{id:"leafy-seadragon",name:"Leafy Seadragon",sci:"Phycodurus eques",depth:26,range:"0–50 m",size:104,side:"left",fact:"A relative of the seahorse, disguised as drifting seaweed by leaf-shaped flaps of skin. It has no teeth and no stomach, sucking up tiny shrimp through its long snout; the male carries the eggs.",svg:`
      ...
      </svg>`},{id:"clownfish",name:"Clownfish",sci:"Amphiprion ocellaris",depth:48,range:"1–15 m",size:84,side:"right",fact:"Lives inside the stinging arms of sea anemones — a coat of mucus keeps it from getting stung. Every clownfish is born male; the dominant one becomes female.",svg:`
      ...
      </svg>`},{id:"moon-jelly",name:"Moon Jelly",sci:"Aurelia aurita",depth:64,range:"0–1,000 m",size:118,side:"left",fact:"Drifts on the currents with almost no ability to swim against them. Its body is about 95% water, and the four pale horseshoes glowing through the bell are its reproductive organs.",svg:`
      ...
      </svg>`},{id:"green-sea-turtle",name:"Green Sea Turtle",sci:"Chelonia mydas",depth:96,range:"Surface–20 m",size:150,side:"right",fact:"One of the few reptiles that grazes like a cow — adults eat seagrass and algae, which tints their body fat green and gives them their name. They can hold their breath for hours while resting.",svg:`
      ...
      </svg>`},{id:"giant-manta",name:"Giant Manta Ray",sci:"Mobula birostris",depth:120,range:"Surface–120 m",size:225,side:"left",fact:"The largest ray on Earth, spanning up to 7 metres wingtip to wingtip. It filters plankton from the water and has the biggest brain of any fish — curious enough to seem to interact with divers.",svg:`
      ...
      </svg>`},{id:"great-white",name:"Great White Shark",sci:"Carcharodon carcharias",depth:150,range:"Surface–250 m",size:235,side:"right",fact:"A warm-bodied hunter that keeps its muscles above the surrounding water temperature, letting it burst after seals in a cold sea. It can smell a single drop of blood in a hundred litres of water.",svg:`
      ...
      </svg>`},{id:"ocean-sunfish",name:"Ocean Sunfish",sci:"Mola mola",depth:175,range:"0–600 m",size:150,side:"left",fact:"The heaviest bony fish in the sea, shaped like a giant swimming head that has lost its tail. It basks on its side at the surface to warm up and let seabirds pick parasites off its skin.",svg:`
      ...
      </svg>`},{id:"whale-shark",name:"Whale Shark",sci:"Rhincodon typus",depth:196,range:"0–1,000 m",size:255,side:"right",fact:"The biggest fish alive, growing longer than a school bus — yet it eats some of the smallest food, filtering plankton from the water. The spot pattern on its skin is unique to each shark, like a fingerprint.",svg:`
      ...
      </svg>`},{id:"firefly-squid",name:"Firefly Squid",sci:"Watasenia scintillans",depth:240,range:"180–370 m",size:82,side:"left",fact:"A hand-sized squid studded with light organs that flash electric blue. Each spring, millions gather to spawn in a Japanese bay, lighting up the waves — one of the ocean’s great glowing spectacles.",svg:`
      ...
      </svg>`},{id:"lanternfish",name:"Lanternfish",sci:"Myctophidae",depth:300,range:"200–1,000 m",size:92,side:"right",fact:"Possibly the most abundant vertebrate on the planet — their combined weight may top a billion tonnes. Rows of light organs dot their bellies, and each night they rise to the surface to feed, then sink back before dawn.",svg:`
      ...
      </svg>`},{id:"snipe-eel",name:"Snipe Eel",sci:"Nemichthys scolopaceus",depth:360,range:"300–600 m",size:190,side:"left",fact:"A ribbon-thin eel with jaws like a bird’s beak that curve apart and can never fully close. The hooked tips snag the antennae of tiny shrimp, which is almost all it eats.",svg:`
      ...
      </svg>`},{id:"humboldt-squid",name:"Humboldt Squid",sci:"Dosidicus gigas",depth:430,range:"200–700 m",size:150,side:"right",fact:"A fast, powerful predator that hunts in coordinated packs and flickers red-and-white to signal to the others. Divers nickname it the “red devil” — it can change colour in a heartbeat.",svg:`
      ...
      </svg>`},{id:"swordfish",name:"Swordfish",sci:"Xiphias gladius",depth:500,range:"Surface–550 m",size:220,side:"left",fact:"Dives into the cold twilight zone to hunt, then returns to sun-warmed surface water to recover. Special tissue near its eyes heats its brain and vision, keeping it sharp in the chill of the deep.",svg:`
      ...
      </svg>`},{id:"cookiecutter-shark",name:"Cookiecutter Shark",sci:"Isistius brasiliensis",depth:560,range:"0–3,700 m",size:150,side:"right",fact:"A small shark that bites round plugs of flesh from animals far larger than itself — even whales and submarines carry its cookie-shaped scars. Its glowing belly lures prey close in the dark.",svg:`
      ...
      </svg>`},{id:"hatchetfish",name:"Hatchetfish",sci:"Sternoptychidae",depth:620,range:"200–1,000 m",size:80,side:"left",fact:"Its thin, mirror-bright body reflects the dim blue light so predators below can’t see its silhouette. Glowing organs along its belly fine-tune the trick, matching the faint glow from above — a disappearing act called counter-illumination.",svg:`
      ...
      </svg>`},{id:"barreleye",name:"Barreleye",sci:"Macropinna microstoma",depth:690,range:"600–800 m",size:118,side:"right",fact:"Its whole forehead is a transparent dome, and the two glowing green orbs inside are its eyes — which rotate to peer straight up through its own skull for prey drifting overhead.",svg:`
      ...
      </svg>`},{id:"bristlemouth",name:"Bristlemouth",sci:"Cyclothone",depth:745,range:"300–2,000 m",size:88,side:"left",fact:"Likely the most numerous vertebrate on Earth — there may be quadrillions of them. Barely finger-length, with a mouth full of bristle-like teeth and rows of tiny glowing dots along its body.",svg:`
      ...
      </svg>`},{id:"vampire-squid",name:"Vampire Squid",sci:"Vampyroteuthis infernalis",depth:820,range:"300–900 m",size:100,side:"right",fact:'Not a vampire at all — it drifts through the low-oxygen twilight zone catching falling "marine snow" (bits of dead plankton) on sticky filaments. When threatened it turns its cloak of arms inside out, baring harmless spines.',svg:`
      ...
      </svg>`},{id:"pacific-viperfish",name:"Pacific Viperfish",sci:"Chauliodus macouni",depth:890,range:"200–1,000 m",size:128,side:"left",fact:"Hunts with fangs so long they won’t fit inside its mouth, curving up past its eyes. A glowing lure on its back fin dangles bait toward the trap of its jaws.",svg:`
      ...
      </svg>`},{id:"giant-squid",name:"Giant Squid",sci:"Architeuthis dux",depth:950,range:"300–1,000 m",size:250,side:"right",fact:"One of the largest animals without a backbone, reaching 12–13 metres including its feeding tentacles. Its eyes are the size of dinner plates — the biggest in the animal kingdom — built to catch the faintest glow in the dark.",svg:`
      ...
      </svg>`},{id:"frilled-shark",name:"Frilled Shark",sci:"Chlamydoselachus anguineus",depth:1080,range:"500–1,500 m",size:190,side:"left",fact:"A living fossil almost unchanged for millions of years, with an eel-like body and six frilly gill slits. Its 300 backward-hooked teeth make escape nearly impossible once prey is caught.",svg:`
      ...
      </svg>`},{id:"bloodybelly-comb-jelly",name:"Bloodybelly Comb Jelly",sci:"Lampocteis cruentiventer",depth:1200,range:"800–1,500 m",size:92,side:"right",fact:"Rows of tiny beating combs scatter light into shimmering rainbows as it swims. Its stomach is blood-red to hide the glow of the bioluminescent prey it swallows.",svg:`
      ...
      </svg>`},{id:"goblin-shark",name:"Goblin Shark",sci:"Mitsukurina owstoni",depth:1300,range:"270–1,300 m",size:205,side:"left",fact:"A pink, slow-moving shark that ambushes prey by shooting its jaws forward on stretchy skin like a slingshot. Its long blade of a snout is packed with sensors for detecting faint electric fields.",svg:`
      ...
      </svg>`},{id:"anglerfish",name:"Anglerfish",sci:"Melanocetus johnsonii",depth:1420,range:"1,000–2,000 m",size:110,side:"right",fact:"Fishes in total darkness with a glowing lure grown from its own head — the light is made by colonies of bacteria living inside it. The tiny male fuses onto the giant female for life, becoming little more than a source of sperm.",svg:`
      ...
      </svg>`},{id:"giant-isopod",name:"Giant Isopod",sci:"Bathynomus giganteus",depth:1550,range:"365–2,300 m",size:122,side:"left",fact:"A deep-sea relative of the woodlouse in your garden, but grown to the size of a house cat. It scavenges whatever sinks from above and can survive years between meals — one in captivity refused food for over five years.",svg:`
      ...
      </svg>`},{id:"black-dragonfish",name:"Black Dragonfish",sci:"Idiacanthus atlanticus",depth:1680,range:"500–2,000 m",size:150,side:"right",fact:"Produces red light from organs under its eyes — a colour almost no other deep-sea animal can see, giving it a private searchlight. Females bristle with fangs and a chin barbel; the tiny males can’t even eat.",svg:`
      ...
      </svg>`},{id:"colossal-squid",name:"Colossal Squid",sci:"Mesonychoteuthis hamiltoni",depth:1750,range:"1,000–2,000 m",size:245,side:"left",fact:"Heavier than the giant squid and armed with swivelling hooks on its arms. It has the largest eyes of any known animal — each the size of a dinner plate — to gather the faintest light in the dark.",svg:`
      ...
      </svg>`},{id:"black-swallower",name:"Black Swallower",sci:"Chiasmodon niger",depth:1900,range:"700–2,700 m",size:110,side:"right",fact:"Can swallow prey more than twice its length and ten times its weight, its stretchy stomach ballooning out below. Sometimes it bites off more than it can digest — and pays the price.",svg:`
      ...
      </svg>`},{id:"sperm-whale",name:"Sperm Whale",sci:"Physeter macrocephalus",depth:2100,range:"Surface–2,250 m",size:258,side:"left",fact:"The deepest-diving of the great whales, plunging beyond 2,000 m and holding its breath for over an hour to hunt squid in the dark. It finds them using the loudest clicks made by any animal.",svg:`
      ...
      </svg>`},{id:"atolla-jelly",name:"Atolla Jelly",sci:"Atolla wyvillei",depth:2350,range:"1,000–4,000 m",size:96,side:"right",fact:"When attacked, it flashes a spinning circle of blue light — a burglar alarm that calls in bigger predators to attack whatever is biting it. It trails one extra-long tentacle out like a fishing line.",svg:`
      ...
      </svg>`},{id:"ghost-shark",name:"Ghost Shark",sci:"Hydrolagus",depth:2550,range:"1,000–2,600 m",size:150,side:"left",fact:"A relative of sharks with a skeleton of cartilage and a single permanent set of grinding tooth-plates. Males have a retractable, spiny club on the forehead used during mating — one of the ocean’s oddest tools.",svg:`
      ...
      </svg>`},{id:"fangtooth",name:"Fangtooth",sci:"Anoplogaster cornuta",depth:2750,range:"2,000–5,000 m",size:92,side:"right",fact:"Has the largest teeth relative to body size of any fish in the ocean — its fangs are so long that they slide into sockets in its own skull so it can close its mouth. Despite the nightmare grin, it’s only about 16 cm long.",svg:`
      ...
      </svg>`},{id:"gulper-eel",name:"Gulper Eel",sci:"Eurypharynx pelecanoides",depth:3e3,range:"500–3,000 m",size:180,side:"left",fact:"Almost all mouth — its enormous, loosely hinged jaw can open like a pelican’s pouch to swallow prey larger than itself. The rest of its body tapers into a whip-thin tail tipped with a glowing pink light.",svg:`
      ...
      </svg>`},{id:"dumbo-octopus",name:"Dumbo Octopus",sci:"Grimpoteuthis",depth:3300,range:"1,000–7,000 m",size:130,side:"right",fact:"The deepest-living octopus known, named for the ear-like fins it flaps to swim. Instead of squirting ink — useless in the dark — it simply flushes red or pale to signal, and swallows its prey whole.",svg:`
      ...
      </svg>`},{id:"grenadier",name:"Grenadier (Rattail)",sci:"Coryphaenoides",depth:4080,range:"2,000–6,000 m",size:190,side:"left",fact:"One of the most common fish of the deep seafloor, with a bulky head tapering to a long rat-like tail. It sniffs out carcasses across vast distances of cold mud, following the scent to a feast.",svg:`
      ...
      </svg>`},{id:"tripod-fish",name:"Tripod Fish",sci:"Bathypterois grallator",depth:4360,range:"900–4,700 m",size:130,side:"right",fact:"Perches motionless on three long, stiffened fin rays like a camera on a tripod, facing into the current. Nearly blind, it waits with its front fins spread wide to feel for tiny prey drifting past in the dark.",svg:`
      ...
      </svg>`},{id:"venus-flower-basket",name:"Venus’ Flower Basket",sci:"Euplectella aspergillum",depth:4620,range:"500–5,000 m",size:108,side:"left",fact:"A glass sponge that weaves a delicate lattice of silica — natural fibre-optic glass. A pair of shrimp often live sealed inside it for life, which is why in Japan it is given as a wedding gift.",svg:`
      ...
      </svg>`},{id:"sea-pig",name:"Sea Pig",sci:"Scotoplanes globosa",depth:4860,range:"1,000–6,000 m",size:110,side:"right",fact:"A plump, pinkish sea cucumber that walks the abyssal mud on stubby water-filled legs, hoovering up the richest sediment like a vacuum. Herds of them gather wherever food falls, sometimes hundreds all facing the same way.",svg:`
      ...
      </svg>`},{id:"sea-spider",name:"Sea Spider",sci:"Colossendeis",depth:5100,range:"1,000–7,000 m",size:132,side:"left",fact:"Not a true spider, but a leggy relative that can span half a metre in cold deep water. It has almost no body — its organs spill out into those spindly legs — and it “breathes” straight through its skin.",svg:`
      ...
      </svg>`},{id:"sea-lily",name:"Sea Lily",sci:"Crinoidea",depth:5340,range:"200–9,000 m",size:116,side:"right",fact:"It looks like a flower but is actually an animal — a relative of the starfish, anchored on a stalk and combing food from the current with feathery arms. Its ancestors filled the seas 500 million years ago.",svg:`
      ...
      </svg>`},{id:"enypniastes",name:"Headless Chicken Monster",sci:"Enypniastes eximia",depth:5580,range:"400–6,000 m",size:122,side:"left",fact:"Nicknamed the “headless chicken monster,” this see-through swimming sea cucumber flaps a fleshy veil to lift off the seabed and drift away from danger. You can watch its gut working through its skin.",svg:`
      ...
      </svg>`},{id:"brittle-star",name:"Brittle Star",sci:"Ophiuroidea",depth:5820,range:"0–7,000 m",size:128,side:"right",fact:"A relative of the starfish with five whip-like arms that ripple to crawl and can snap off to escape a predator, then regrow. On some deep plains they carpet the seafloor in the millions.",svg:`
      ...
      </svg>`},{id:"abyssobrotula",name:"Deep-sea Cusk Eel",sci:"Abyssobrotula galatheae",depth:6500,range:"3,000–8,400 m",size:175,side:"left",fact:"A pale, near-blind cusk-eel that holds one of the deepest fish records ever — one was hauled up from 8,370 m in the Puerto Rico Trench. It feels its way through the mud for tiny prey it cannot see.",svg:`
      ...
      </svg>`},{id:"hadal-anemone",name:"Trench Anemone",sci:"Galatheanthemum",depth:7100,range:"6,000–10,700 m",size:96,side:"right",fact:"Anemones like this anchor to rocks miles down inside ocean trenches, spreading a crown of tentacles to snare whatever drifts by. Some trench species were only discovered by robot landers dropped into the deep.",svg:`
      ...
      </svg>`},{id:"mariana-snailfish",name:"Mariana Snailfish",sci:"Pseudoliparis swirei",depth:7700,range:"6,900–8,000 m",size:122,side:"left",fact:"The deepest fish ever filmed, thriving where the pressure would crush a submarine. Its body has almost no scales and a soft, translucent, tadpole-like shape — a special molecule in its cells keeps its proteins from being squeezed apart.",svg:`
      ...
      </svg>`},{id:"elpidia",name:"Trench Sea Cucumber",sci:"Elpidia",depth:8300,range:"6,000–9,500 m",size:100,side:"right",fact:"A small, pale sea cucumber that dominates the very deepest trenches, plowing through the sediment to strip out the last scraps of food. Where almost nothing else survives, these thrive in slow-moving herds.",svg:`
      ...
      </svg>`},{id:"deepstaria",name:"Deepstaria Jelly",sci:"Deepstaria enigmatica",depth:8900,range:"1,000–9,000 m",size:130,side:"left",fact:"A ghostly jelly whose enormous, paper-thin bell unfurls like a living net or billowing blanket to trap prey. Filmed only rarely, it can draw itself closed into a bag around whatever it catches.",svg:`
      ...
      </svg>`},{id:"giant-amphipod",name:"Giant Amphipod",sci:"Hirondellea gigas",depth:9600,range:"6,000–11,000 m",size:100,side:"right",fact:"A shrimp-like scavenger that swarms the very bottom of the Mariana Trench, feasting on anything that falls. It even eats sunken wood, using gut enzymes to digest it — and armours its shell with aluminium pulled from the seawater.",svg:`
      ...
      </svg>`},{id:"xenophyophore",name:"Xenophyophore",sci:"Syringammina fragilissima",depth:10500,range:"500–10,600 m",size:110,side:"left",fact:'Here’s the "whoa": this fist-sized lump is a single cell — one of the largest single-celled organisms alive. It builds its own delicate shell by gluing together grains of sediment, and whole gardens of them carpet the trench floor.',svg:`
      ...
      </svg>`},{id:"cuttlefish",name:"Common Cuttlefish",sci:"Sepia officinalis",depth:80,range:"0–200 m",size:120,fact:"A master of disguise that changes the colour and texture of its skin in a blink using millions of tiny pigment sacs — even though, strangely, it is probably colour-blind. It has three hearts and blue-green blood.",svg:`
      ...
      </svg>`},{id:"hammerhead",name:"Great Hammerhead",sci:"Sphyrna mokarran",depth:230,range:"0–300 m",size:220,fact:"Its wide, flattened head is studded with sensors that detect the faint electric fields of animals buried in the sand — sweeping it like a metal detector to pin stingrays it then eats.",svg:`
      ...
      </svg>`},{id:"opah",name:"Opah",sci:"Lampris guttatus",depth:280,range:"50–500 m",size:180,fact:"The first fish ever found to be fully warm-blooded — it heats its whole body by circulating warm blood, so it stays fast and sharp while hunting in cold, deep water where other fish turn sluggish.",svg:`
      ...
      </svg>`},{id:"oarfish",name:"Giant Oarfish",sci:"Regalecus glesne",depth:340,range:"200–1,000 m",size:240,fact:"The longest bony fish in the sea, a silvery ribbon that can stretch 8 metres with a flowing red crest. Rarely seen alive, its serpentine body is almost certainly behind many old tales of sea monsters.",svg:`
      ...
      </svg>`},{id:"strawberry-squid",name:"Strawberry Squid",sci:"Histioteuthis heteropsis",depth:420,range:"200–1,000 m",size:120,fact:"Has two wildly mismatched eyes: one is huge and looks up to spot shadows against the faint light above, while the other is small and points down, watching for the sparkle of glowing prey below.",svg:`
      ...
      </svg>`},{id:"sixgill-shark",name:"Bluntnose Sixgill Shark",sci:"Hexanchus griseus",depth:500,range:"200–1,100 m",size:230,fact:"An ancient giant with six gill slits instead of the usual five, and a single fin set far back — a body plan little changed since before the dinosaurs. It rises from the depths at night to hunt.",svg:`
      ...
      </svg>`},{id:"pyrosome",name:"Pyrosome",sci:"Pyrosoma atlanticum",depth:560,range:"0–1,000 m",size:150,fact:"Not one animal but thousands of tiny clones fused into a single glowing tube, open at one end. Each clone filters food and, when touched, the whole colony lights up in rippling waves of blue-green light.",svg:`
      ...
      </svg>`},{id:"lancetfish",name:"Longnose Lancetfish",sci:"Alepisaurus ferox",depth:620,range:"0–1,800 m",size:200,fact:"A fanged predator with a tall sail on its back and a soft, watery body that can’t chase fast. Instead it swallows prey whole and digests slowly — and its stomach is so often intact that scientists study it to learn what lives in the deep.",svg:`
      ...
      </svg>`},{id:"hagfish",name:"Pacific Hagfish",sci:"Eptatretus stoutii",depth:680,range:"100–900 m",size:150,fact:"When a predator bites, it instantly unleashes a cloud of slime that swells into litres of choking goo, clogging the attacker’s gills. Jawless and nearly blind, it ties itself in a knot to scrape the slime back off.",svg:`
      ...
      </svg>`},{id:"telescope-octopus",name:"Telescope Octopus",sci:"Amphitretus pelagicus",depth:740,range:"150–2,000 m",size:120,fact:"Almost perfectly transparent and the only octopus with tubular, telescope-like eyes on stalks. They swivel to point upward, scanning the faint glow above for the silhouettes of passing prey.",svg:`
      ...
      </svg>`},{id:"siphonophore",name:"Siphonophore",sci:"Marrus orthocanna",depth:1010,range:"200–2,000 m",size:130,fact:"Looks like a single jelly but is really a colony of specialized clones strung together, each doing one job — floating, swimming, feeding, or stinging. Some relatives grow longer than a blue whale.",svg:`
      ...
      </svg>`},{id:"glass-squid",name:"Glass Squid",sci:"Cranchiidae",depth:860,range:"200–1,000 m",size:120,fact:"Nearly invisible — its whole body is see-through except for its eyes and gut. To hide even those telltale shadows, it shines light organs beneath its eyes to erase its silhouette from below.",svg:`
      ...
      </svg>`},{id:"salp",name:"Salp",sci:"Salpa maxima",depth:900,range:"0–800 m",size:110,fact:"A see-through, barrel-shaped animal that jets water through its body to move and feed at once, often linking into long glowing chains. As it eats, it packages carbon and sinks it to the deep — a tiny cog in cooling the planet.",svg:`
      ...
      </svg>`},{id:"cutthroat-eel",name:"Cutthroat Eel",sci:"Synaphobranchidae",depth:940,range:"300–3,700 m",size:160,fact:"Named for gill slits set low on the throat like a slash. These eels are among the first to arrive at anything dead that sinks to the deep seafloor, swarming bait traps by the dozen.",svg:`
      ...
      </svg>`},{id:"phronima",name:"Pram Bug",sci:"Phronima sedentaria",depth:780,range:"200–1,000 m",size:92,fact:"A see-through amphipod that hollows out a jelly-like salp and rides inside the barrel, pushing it through the water like a pram while it raises its young. Its eerie look helped inspire the creature in the film Alien.",svg:`
      ...
      </svg>`},{id:"lizardfish",name:"Deep-sea Lizardfish",sci:"Bathysaurus ferox",depth:1080,range:"1,000–3,500 m",size:170,fact:"A ferocious ambush hunter of the deep seafloor whose mouth, tongue and even the roof of its mouth are lined with hinged, backward-pointing teeth — so once it grabs a meal, nothing gets back out.",svg:`
      ...
      </svg>`},{id:"pigbutt-worm",name:"Pigbutt Worm",sci:"Chaetopterus pugaporcinus",depth:1180,range:"~1,000 m",size:92,fact:"A marble-sized deep-sea worm that puffs its middle segments into a glowing balloon shaped, unmistakably, like a tiny pair of buttocks. It drifts in a cloud of mucus that traps falling food.",svg:`
      ...
      </svg>`},{id:"bigfin-squid",name:"Bigfin Squid",sci:"Magnapinna",depth:1300,range:"1,000–4,700 m",size:200,fact:'One of the ocean’s strangest sights, filmed only a handful of times. It holds its impossibly long arms bent at the "elbow" and out at right angles, trailing them like sticky threads many metres below its small body.',svg:`
      ...
      </svg>`},{id:"footballfish",name:"Football Fish",sci:"Himantolophus groenlandicus",depth:1420,range:"1,000–2,000 m",size:130,fact:"A round, spiky anglerfish that dangles a glowing bulb over its mouth to draw prey in. Only the football-sized female fishes; the tiny male is a fraction of her size and latches on to live off her body.",svg:`
      ...
      </svg>`},{id:"loosejaw",name:"Stoplight Loosejaw",sci:"Malacosteus niger",depth:1520,range:"500–3,000 m",size:130,fact:"Shines a secret red searchlight from organs under its eyes — a colour nearly every other deep animal is blind to — so it can spot prey unseen. Its lower jaw has no floor of skin, snapping shut like a bare cage of teeth.",svg:`
      ...
      </svg>`},{id:"crossota",name:"Crossota Jelly",sci:"Crossota millsae",depth:1640,range:"1,000–3,000 m",size:92,fact:"A small, deep-red jelly that spends its whole life in open water, never touching the seafloor. Dozens of fine tentacles fan out around its bell as it pulses slowly through the eternal night.",svg:`
      ...
      </svg>`},{id:"whipnose",name:"Whipnose Anglerfish",sci:"Gigantactis",depth:1740,range:"1,000–2,700 m",size:130,fact:"Its fishing rod is longer than its whole body, dangling a glowing lure far out front to tempt prey back toward the waiting mouth. Oddly, it is often seen swimming upside-down, dragging the lure along the seabed.",svg:`
      ...
      </svg>`},{id:"coffinfish",name:"Coffinfish",sci:"Chaunax",depth:1850,range:"200–2,500 m",size:120,fact:'A pudgy "sea toad" covered in prickly skin that lumbers over the seabed on arm-like fins. It can gulp in huge mouthfuls of seawater and hold its breath for minutes, saving energy where food is scarce.',svg:`
      ...
      </svg>`},{id:"glass-octopus",name:"Glass Octopus",sci:"Vitreledonella richardi",depth:1960,range:"1,000–3,000 m",size:130,fact:"Almost completely transparent — only its cylindrical eyes, its gut and its optic nerves are visible, everything else clear as water. This near-invisibility is its best defence in a place with nowhere to hide.",svg:`
      ...
      </svg>`},{id:"paper-lantern-jelly",name:"Red Paper Lantern Jelly",sci:"Pandea rubra",depth:2080,range:"1,000–2,000 m",size:100,fact:"A deep-red jelly that can crumple and pleat its bell like a folding paper lantern, then smooth itself back out to drift on. Small shrimp sometimes hitch a ride inside its stomach.",svg:`
      ...
      </svg>`},{id:"zombie-worm",name:"Zombie Worm",sci:"Osedax",depth:2200,range:"1,000–3,000 m",size:92,fact:'Has no mouth, no gut, and no stomach. It bores into the bones of dead whales with acid and lets bacteria inside it digest the fat — earning the nickname "bone-eating snot flower." Only the females feed; the males live tiny inside them.',svg:`
      ...
      </svg>`},{id:"eelpout",name:"Deep-sea Eelpout",sci:"Zoarcidae",depth:2320,range:"1,000–2,700 m",size:150,fact:"One of the few fish families that thrives around scalding hydrothermal vents, grazing on the shrimp and crabs that swarm the mineral chimneys — living between water hot enough to melt lead and the near-freezing deep.",svg:`
      ...
      </svg>`},{id:"cusk-eel",name:"Deep-sea Cusk-eel",sci:"Bassozetus",depth:2450,range:"1,500–4,000 m",size:160,fact:"A soft, tapering fish whose fins merge into one long ribbon running around its body. Members of this family have been caught deeper than almost any other fish, right down into the ocean trenches.",svg:`
      ...
      </svg>`},{id:"bathocyroe",name:"Comb Jelly",sci:"Bathocyroe fosteri",depth:2560,range:"600–2,500 m",size:92,fact:"One of the most common jellies of the deep, it swims by beating eight rows of tiny hair-like combs. As they beat, they scatter light into shimmering, moving rainbows — even though the animal itself makes no colour.",svg:`
      ...
      </svg>`},{id:"helmet-jelly",name:"Helmet Jellyfish",sci:"Periphylla periphylla",depth:2680,range:"1,000–2,700 m",size:110,fact:"A deep-red, helmet-shaped jelly so sensitive to light that sunlight can be deadly to it — so it hides in the dark deep by day. When disturbed it flashes with blue-green bioluminescence.",svg:`
      ...
      </svg>`},{id:"solmissus",name:"Dinnerplate Jelly",sci:"Solmissus",depth:2900,range:"1,000–3,000 m",size:120,fact:'Nicknamed the "dinnerplate jelly," it cruises through the dark holding its tentacles stiffly out ahead like the spokes of a wheel — an unusual hunter that ambushes and eats other jellies.',svg:`
      ...
      </svg>`},{id:"bamboo-coral",name:"Bamboo Coral",sci:"Isididae",depth:3050,range:"1,000–4,000 m",size:130,fact:"A deep-sea coral that grows in jointed segments like a stalk of bamboo and can live for thousands of years, laying down rings that record the ocean’s past — a slow-growing forest in the cold and dark.",svg:`
      ...
      </svg>`},{id:"harp-sponge",name:"Harp Sponge",sci:"Chondrocladia lyra",depth:3350,range:"3,300–3,500 m",size:130,fact:"A carnivorous sponge shaped like a harp or candelabra. Instead of filtering water, its rows of upright branches are covered in tiny hooks that snag drifting shrimp, which it slowly wraps in membrane and digests.",svg:`
      ...
      </svg>`},{id:"flabby-whalefish",name:"Flabby Whalefish",sci:"Cetomimidae",depth:3550,range:"1,000–4,000 m",size:150,fact:"The loose-skinned, tiny-eyed female looks so unlike the big-eyed male and the larva that scientists once sorted the three into completely different families — until DNA revealed they are one fish at three life stages.",svg:`
      ...
      </svg>`},{id:"vent-shrimp",name:"Vent Shrimp",sci:"Rimicaris exoculata",depth:2250,range:"2,000–2,600 m",size:100,fact:'Swarms in the thousands over scalding hydrothermal vents. It has no normal eyes — instead a light-sensing patch on its back "sees" the faint glow of the superheated water, keeping it close to the warmth without being cooked.',svg:`
      ...
      </svg>`},{id:"yeti-crab",name:"Yeti Crab",sci:"Kiwa hirsuta",depth:2200,range:"2,200–2,600 m",size:120,fact:'A blind, pale crab discovered near hydrothermal vents, its arms covered in silky "fur." It farms bacteria on those hairs and waves its claws through the mineral-rich water to feed its living crop, then eats it.',svg:`
      ...
      </svg>`},{id:"faceless-cusk",name:"Faceless Cusk",sci:"Typhlonus nasus",depth:4100,range:"3,900–5,000 m",size:150,fact:"Rediscovered off Australia in 2017 after more than a century, it has no visible eyes and a mouth tucked underneath, so head-on it appears to have no face at all — just smooth, pale skin.",svg:`
      ...
      </svg>`},{id:"gummy-squirrel",name:"Gummy Squirrel",sci:"Psychropotes longicauda",depth:4250,range:"4,000–6,000 m",size:130,fact:'A big yellow sea cucumber with a "sail" on its back and a long upright tail, it plods across the abyssal plain slurping up the freshest, most nutritious mud off the very top of the seabed.',svg:`
      ...
      </svg>`},{id:"basket-star",name:"Basket Star",sci:"Gorgonocephalus",depth:4400,range:"100–4,500 m",size:130,fact:"A relative of the brittle star whose five arms fork again and again into a living net of thousands of curling tips. At night it raises the whole basket into the current to snare drifting plankton.",svg:`
      ...
      </svg>`},{id:"relicanthus",name:"Giant Anemone",sci:"Relicanthus daphneae",depth:4600,range:"~4,000 m",size:120,fact:"An anemone-like giant with tentacles up to 2 metres long. It anchors to the little dark manganese nodules scattered across the abyssal plain — potato-sized lumps that take millions of years to grow one layer.",svg:`
      ...
      </svg>`},{id:"benthocodon",name:"Red Bottom Jelly",sci:"Benthocodon",depth:4750,range:"4,000–5,000 m",size:100,fact:"Unusual for a jelly, its bell is a solid, opaque blood-red instead of clear — thought to mask the glow of the bioluminescent food it swallows, so predators can’t spot the light through its stomach.",svg:`
      ...
      </svg>`},{id:"munnopsid",name:"Long-legged Isopod",sci:"Munnopsidae",depth:4900,range:"1,000–5,000 m",size:110,fact:"A spidery deep-sea cousin of the pill-bug that leads a double life: it swims upside-down through the water with paddle-shaped legs, then flips over to stalk across the mud on long, stilt-like limbs.",svg:`
      ...
      </svg>`},{id:"cirrate-octopus",name:"Blind Octopus",sci:"Cirrothauma murrayi",depth:5050,range:"1,500–5,000 m",size:120,fact:"One of the only truly blind octopuses. Living in absolute darkness, it has given up working eyes altogether and drifts on two large fins like a gelatinous parachute, feeling for food with webbed arms.",svg:`
      ...
      </svg>`},{id:"brisingid",name:"Deep-sea Sea Star",sci:"Brisingida",depth:5250,range:"2,000–6,000 m",size:130,fact:"Unlike the starfish of shallow reefs, it doesn’t crawl after food. It perches on rocks with many long, spiny arms held up into the current, catching drifting particles on thousands of tiny pincers.",svg:`
      ...
      </svg>`},{id:"graneledone",name:"Deep-sea Octopus",sci:"Graneledone",depth:5450,range:"1,000–5,000 m",size:120,fact:"A warty, pale-purple octopus of the cold deep. A female of a close relative was watched guarding her eggs on a rocky ledge for four and a half years — the longest brooding of any known animal.",svg:`
      ...
      </svg>`},{id:"alicella",name:"Supergiant Amphipod",sci:"Alicella gigantea",depth:5600,range:"3,000–7,000 m",size:110,fact:'The largest amphipod ever found, reaching 34 cm — a true "supergiant" among cousins usually smaller than a grain of rice. It roams the deep scavenging carcasses, growing huge where food is rare but competition is rarer.',svg:`
      ...
      </svg>`},{id:"peniagone",name:"Swimming Sea Cucumber",sci:"Peniagone",depth:6300,range:"5,000–9,500 m",size:110,side:"left",fact:"Many of the deepest sea cucumbers can swim — this one pushes off the mud and undulates a fleshy veil to rise and drift, letting it flee disturbances and ride weak currents to the next patch of food.",svg:`
      ...
      </svg>`},{id:"kermadec-snailfish",name:"Kermadec Snailfish",sci:"Notoliparis kermadecensis",depth:7e3,range:"6,000–7,500 m",size:120,side:"right",fact:"The snailfish of the Kermadec Trench near New Zealand — a whole different trench from the Mariana, yet home to its own pale, jelly-bodied snailfish. Across the world’s deepest trenches, it is snailfish that rule.",svg:`
      ...
      </svg>`},{id:"benthesicymus",name:"Deep-sea Shrimp",sci:"Benthesicymus",depth:7600,range:"4,000–8,000 m",size:120,side:"left",fact:"A red-shelled shrimp that scavenges among the deepest trenches. Red is the first colour to vanish in the deep, so in the lightless water its bright shell actually reads as invisible black — perfect camouflage.",svg:`
      ...
      </svg>`},{id:"scale-worm",name:"Hadal Scale Worm",sci:"Polynoidae",depth:8200,range:"6,000–8,500 m",size:100,side:"right",fact:"Armoured along its back with overlapping plates like roof tiles and fringed with stiff bristles, this bristle worm hunts and scavenges on the trench floor — among the deepest predators known on Earth.",svg:`
      ...
      </svg>`}],re="itd-quiz-best",ge=e=>Math.floor(Math.random()*e);function Q(e){const t=[...e];for(let i=t.length-1;i>0;i--){const a=ge(i+1);[t[i],t[a]]=[t[a],t[i]]}return t}const $=(e,t)=>Q(e).slice(0,t),ee=e=>e.toLowerCase().replace(/[^a-z]/g,"");function C(e,t,i,a){const s=ee(t[a]),o=e.filter(r=>{if(r.id===t.id)return!1;const n=ee(r[a]);return!(n===s||n.slice(0,9)===s.slice(0,9))});return $(o,i)}const we=[["Which animal has the largest eyes of any creature on Earth — each the size of a dinner plate?","colossal-squid"],["Which is the deepest-living fish ever filmed?","mariana-snailfish"],["Which was the first fish discovered to be fully warm-blooded?","opah"],['Which is the longest bony fish, likely behind old "sea serpent" legends?',"oarfish"],["Which is the biggest fish alive, yet feeds on tiny plankton?","whale-shark"],["Which is the heaviest bony fish, shaped like a giant swimming head?","ocean-sunfish"],["Which fist-sized deep-sea creature is a single giant cell?","xenophyophore"],["Which worm has no mouth or gut and bores into whale bones to feed?","zombie-worm"],["Which pale, blind crab farms bacteria on its hairy claws near hot vents?","yeti-crab"],["Which fish has a see-through head with glowing green tube-eyes inside?","barreleye"],["Which shark shoots its jaws forward like a slingshot to grab prey?","goblin-shark"],["Which small shark bites round, cookie-shaped plugs out of whales?","cookiecutter-shark"],['Which see-through swimming sea cucumber is nicknamed the "headless chicken monster"?',"enypniastes"],["Which fish has the largest teeth, for its body size, of any fish in the ocean?","fangtooth"],["Which deep-diving whale hunts squid using the loudest clicks of any animal?","sperm-whale"],["Which glass sponge is given as a wedding gift in Japan because shrimp pairs live inside it for life?","venus-flower-basket"],['Which jelly flashes a spinning "burglar alarm" of blue light when attacked?',"atolla-jelly"],["Which deep octopus flaps ear-like fins to swim and is the deepest-living octopus known?","dumbo-octopus"]],ue=e=>y.find(t=>t.id===e),D=e=>T(e.depth).name;function me(e){const t=Q([e,...C(y,e,3,"name")]);return{kind:"picture",prompt:"Which creature is this?",sprite:e.svg,options:t.map(i=>i.name),correct:t.findIndex(i=>i.id===e.id),explain:e.fact}}function ye(e){const t=D(e),i=G.map(s=>s.name).filter(s=>s!==t),a=Q([t,...$(i,3)]);return{kind:"zone",prompt:`Which ocean zone is home to the ${e.name}?`,sprite:null,options:a,correct:a.indexOf(t),explain:`The ${e.name} lives at about ${Math.round(e.depth).toLocaleString("en-US")} m — in the ${t} zone.`}}function Qe(e){const t=Q([e,...C(y,e,3,"sci")]);return{kind:"sci",prompt:`What is the scientific name of the ${e.name}?`,sprite:null,italic:!0,options:t.map(i=>i.sci),correct:t.findIndex(i=>i.id===e.id),explain:`The ${e.name}’s scientific name is ${e.sci}.`}}function ke(){let e;for(let s=0;s<30;s++){e=$(y,4);const o=e.map(r=>r.depth).sort((r,n)=>r-n);if(o.every((r,n)=>n===0||r-o[n-1]>350))break}const t=Math.random()<.5,i=e.reduce((s,o)=>t?o.depth>s.depth?o:s:o.depth<s.depth?o:s),a=Q(e);return{kind:"depth",prompt:`Which of these lives the ${t?"deepest":"shallowest"}?`,sprite:null,options:a.map(s=>s.name),correct:a.findIndex(s=>s.id===i.id),explain:`The ${i.name} lives at about ${Math.round(i.depth).toLocaleString("en-US")} m — the ${t?"deepest":"shallowest"} of the four.`}}function be([e,t]){const i=ue(t);if(!i)return null;const a=y.filter(r=>D(r)===D(i));let s=C(a,i,3,"name");s.length<3&&(s=s.concat(C(y,i,3-s.length,"name")));const o=Q([i,...s]);return{kind:"trivia",prompt:e,sprite:null,options:o.map(r=>r.name),correct:o.findIndex(r=>r.id===i.id),explain:i.fact}}function ve(e=50){const t=Q(y),i={picture:14,zone:10,sci:8,depth:6,trivia:12},a=t.slice(0,i.picture).map(me),s=t.slice(14,14+i.zone).map(ye),o=t.slice(30,30+i.sci).map(Qe),r=Array.from({length:i.depth},ke),n=$(we,i.trivia).map(be).filter(Boolean),d=Q([...a,...s,...o,...r,...n]);for(let p=1;p<d.length;p++)if(d[p].kind===d[p-1].kind){const g=d.findIndex((x,w)=>w>p&&x.kind!==d[p-1].kind);g>-1&&([d[p],d[g]]=[d[g],d[p]])}return d.slice(0,e)}const W=()=>Number(localStorage.getItem(re)||0);function xe(e){const t=W();return e>t?(localStorage.setItem(re,String(e)),!0):!1}function Me(e){return e>=.92?{title:"Abyssal Master",note:"The deep holds no secrets from you."}:e>=.78?{title:"Deep Diver",note:"Serious pressure, serious knowledge."}:e>=.6?{title:"Twilight Explorer",note:"You know your way down there."}:e>=.4?{title:"Snorkeler",note:"A good start — keep exploring!"}:{title:"Beachcomber",note:"Scroll the ocean and try again!"}}function Le(){const t=document.createElement("button");t.id="quiz-toggle",t.type="button",t.setAttribute("aria-label","Take the creature quiz"),t.innerHTML=`
    ...
    <span>QUIZ</span>`,document.body.appendChild(t);const i=document.createElement("div");i.id="quiz",i.hidden=!0,i.innerHTML=`
    <div class="quiz-panel" role="dialog" aria-modal="true" aria-label="Creature quiz">
    ...
    </div>`,document.body.appendChild(i);const a=i.querySelector(".quiz-body");let s=[],o=0,r=0,n=!1,d=null;function p(){d=document.activeElement,i.hidden=!1,document.body.style.overflow="hidden",requestAnimationFrame(()=>i.classList.add("open")),x()}function g(){i.classList.remove("open"),document.body.style.overflow="";const c=()=>{i.hidden=!0,i.removeEventListener("transitionend",c)};i.addEventListener("transitionend",c),d&&d.focus()}function x(){const c=W();a.className="quiz-body quiz-view-intro",a.innerHTML=`
      <div class="quiz-intro">
        ...
        <h2>Creature Quiz</h2>
        <p class="quiz-lead">50 questions drawn from all 100 creatures — pictures, depths, zones and jaw-dropping facts. How deep does your knowledge go?</p>
        ${c?`<p class="quiz-best">Your best: <strong>${c} / 50</strong></p>`:""}
        <button class="quiz-start" type="button">Start the dive</button>
      </div>`,a.querySelector(".quiz-start").addEventListener("click",w)}function w(){s=ve(50),o=0,r=0,M()}function M(){n=!1;const c=s[o],u=Math.round(o/s.length*100);a.className="quiz-body quiz-view-question",a.innerHTML=`
      <div class="quiz-top">
        <div class="quiz-bar"><span style="width:${u}%"></span></div>
        <div class="quiz-meta">
          <span>Question ${o+1} / ${s.length}</span>
          <span class="quiz-score">Score ${r}</span>
        </div>
      </div>
      ${c.sprite?`<div class="quiz-stage"><div class="quiz-sprite">${c.sprite}</div></div>`:""}
      <h3 class="quiz-prompt">${c.prompt}</h3>
      <div class="quiz-options ${c.italic?"is-sci":""}">
        ${c.options.map((m,k)=>`<button class="quiz-opt" type="button" data-i="${k}">${m}</button>`).join("")}
      </div>
      <div class="quiz-feedback" hidden></div>`,a.querySelectorAll(".quiz-opt").forEach(m=>m.addEventListener("click",()=>I(Number(m.dataset.i))))}function I(c){if(n)return;n=!0;const u=s[o],m=a.querySelectorAll(".quiz-opt"),k=c===u.correct;k&&r++,m.forEach((j,X)=>{j.disabled=!0,X===u.correct?j.classList.add("is-correct"):X===c&&j.classList.add("is-wrong")});const S=a.querySelector(".quiz-feedback"),de=o===s.length-1;S.hidden=!1,S.innerHTML=`
      <p class="quiz-verdict ${k?"ok":"no"}">${k?"Correct!":"Not quite"}</p>
      <p class="quiz-explain">${u.explain}</p>
      <button class="quiz-next" type="button">${de?"See results":"Next question"}</button>`,a.querySelector(".quiz-score").textContent=`Score ${r}`,S.querySelector(".quiz-next").addEventListener("click",ce),S.scrollIntoView({behavior:"smooth",block:"nearest"})}function ce(){o++,o>=s.length?ne():M()}function ne(){const c=r/s.length,u=Me(c),m=xe(r),k=Math.round(c*100);a.className="quiz-body quiz-view-result",a.innerHTML=`
      <div class="quiz-result">
        <div class="quiz-ring" style="--pct:${k}">
          <div class="quiz-ring-inner"><strong>${r}</strong><span>/ ${s.length}</span></div>
        </div>
        <span class="quiz-kicker">${m?"New personal best!":`Best: ${W()} / ${s.length}`}</span>
        <h2>${u.title}</h2>
        <p class="quiz-lead">${u.note}</p>
        <div class="quiz-actions">
          <button class="quiz-restart" type="button">Play again</button>
          <button class="quiz-exit" type="button">Back to the ocean</button>
        </div>
      </div>`,a.querySelector(".quiz-restart").addEventListener("click",w),a.querySelector(".quiz-exit").addEventListener("click",g)}t.addEventListener("click",p),i.querySelector(".quiz-close").addEventListener("click",g),document.addEventListener("keydown",c=>{c.key==="Escape"&&!i.hidden&&g()})}const N=[...y].sort((e,t)=>e.depth-t.depth);N.forEach((e,t)=>{e.side=t%2===0?"right":"left"});const b=document.getElementById("ocean"),A=document.documentElement,L={depth:document.getElementById("hud-depth"),zone:document.getElementById("hud-zone"),zoneSci:document.getElementById("hud-zone-sci"),light:document.getElementById("hud-light"),pressure:document.getElementById("hud-pressure")},R=F*z;b.style.height=`calc(${R}px + 100svh)`;function J(e){return`calc(${Math.round(e*z)}px + 50svh)`}const B=document.createElement("div");B.id="ruler";B.setAttribute("aria-hidden","true");B.style.top="50svh";B.style.height=`${R}px`;b.appendChild(B);for(let e=500;e<=F;e+=500){const t=document.createElement("span");t.className="ruler-label",t.textContent=`${e.toLocaleString("en-US")} m`,t.style.top=J(e),b.appendChild(t)}for(const e of G.slice(1)){const t=document.createElement("div");t.className="zone-marker",t.style.top=J(e.from),t.innerHTML=`
    <div class="zone-marker-text">
      ${e.name} Zone · ${e.from.toLocaleString("en-US")} m
      <small>${e.sci}</small>
    </div>`,b.appendChild(t)}const le=new IntersectionObserver(e=>{for(const t of e)t.isIntersecting&&(t.target.classList.add("in-view"),le.unobserve(t.target))},{rootMargin:"0px 0px -12% 0px"});for(const e of N){const t=document.createElement("article");t.className=e.depth>=1e3?"creature deep":"creature",t.dataset.side=e.side,t.dataset.id=e.id,t.style.top=J(e.depth),t.innerHTML=`
    <button class="creature-tap" type="button" aria-label="Open details for ${e.name}">
      <div class="creature-sprite" style="width:${e.size}px">${e.svg}</div>
    </button>
    <div class="creature-label">
      <h3>${e.name}</h3>
      <em>${e.sci}</em>
      <span class="creature-range">${e.range}</span>
    </div>`,t.querySelector(".creature-tap").addEventListener("click",()=>Ge(e)),b.appendChild(t),le.observe(t)}const l=document.createElement("div");l.id="detail";l.hidden=!0;l.innerHTML=`
  ...
  </div>`;document.body.appendChild(l);const h={sprite:l.querySelector(".detail-sprite"),zone:l.querySelector("#detail-zone"),zonefull:l.querySelector("#detail-zonefull"),name:l.querySelector("#detail-name"),sci:l.querySelector("#detail-sci"),range:l.querySelector("#detail-range"),fact:l.querySelector("#detail-fact"),card:l.querySelector(".detail-card")};let P=null;function Ge(e){const t=T(e.depth);h.sprite.innerHTML=e.svg,h.sprite.style.maxWidth=`${Math.min(e.size*1.7,260)}px`,h.zone.textContent=`${t.name} Zone`,h.zonefull.textContent=`${t.name} · ${t.sci}`,h.name.textContent=e.name,h.sci.textContent=e.sci,h.range.textContent=e.range,h.fact.textContent=e.fact,l.dataset.zone=t.name.toLowerCase(),P=document.activeElement,l.hidden=!1,document.body.style.overflow="hidden",requestAnimationFrame(()=>l.classList.add("open")),l.querySelector(".detail-close").focus()}function U(){l.classList.remove("open"),document.body.style.overflow="";const e=()=>{l.hidden=!0,l.removeEventListener("transitionend",e)};l.addEventListener("transitionend",e),P&&P.focus()}l.addEventListener("click",e=>{e.target.hasAttribute("data-close")&&U()});document.addEventListener("keydown",e=>{e.key==="Escape"&&!l.hidden&&U()});let Z=null;h.card.addEventListener("touchstart",e=>{Z=e.touches[0].clientY},{passive:!0});h.card.addEventListener("touchmove",e=>{if(Z===null)return;const t=e.touches[0].clientY-Z;t>0&&(h.card.style.transform=`translateY(${Math.min(t,200)}px)`)},{passive:!0});h.card.addEventListener("touchend",e=>{const t=e.changedTouches[0].clientY-(Z??0);h.card.style.transform="",t>90&&U(),Z=null},{passive:!0});const v=document.createElement("button");v.id="divemap-toggle";v.type="button";v.setAttribute("aria-label","Open the dive line to jump to a creature");v.innerHTML=`
  ...
  <span>DIVE<br/>LINE</span>`;document.body.appendChild(v);const f=document.createElement("div");f.id="divemap";f.hidden=!0;f.innerHTML=`
  ...
  </aside>`;document.body.appendChild(f);const te=f.querySelector(".divemap-track");let ie=null;for(const e of N){const t=T(e.depth);if(t.name!==ie){ie=t.name;const a=document.createElement("div");a.className="divemap-zone",a.dataset.zone=t.name.toLowerCase();const s=t.to>=11e3?"10,935":t.to.toLocaleString("en-US");a.innerHTML=`<span>${t.name} · ${t.from.toLocaleString("en-US")}–${s} m</span>`,te.appendChild(a)}const i=document.createElement("button");i.type="button",i.className="divemap-item",i.dataset.side=e.side,i.setAttribute("aria-label",`Jump to ${e.name} at ${Math.round(e.depth)} metres`),i.innerHTML=`
    <span class="dm-chip">
      <span class="dm-node">${e.svg}</span>
      <span class="dm-label"><b>${e.name}</b><i>${Math.round(e.depth).toLocaleString("en-US")} m</i></span>
    </span>`,i.addEventListener("click",()=>Ze(e)),te.appendChild(i)}function ze(){f.hidden=!1,document.body.style.overflow="hidden",requestAnimationFrame(()=>f.classList.add("open"))}function V(){f.classList.remove("open"),document.body.style.overflow="";const e=()=>{f.hidden=!0,f.removeEventListener("transitionend",e)};f.addEventListener("transitionend",e)}function Ze(e){V();const t=window.matchMedia("(prefers-reduced-motion: reduce)").matches;window.scrollTo({top:Math.round(e.depth*z),behavior:t?"auto":"smooth"})}v.addEventListener("click",ze);f.addEventListener("click",e=>{e.target.hasAttribute("data-close")&&V()});document.addEventListener("keydown",e=>{e.key==="Escape"&&!f.hidden&&V()});const Y=document.createElement("div");Y.id="floor";Y.innerHTML=`
  <div class="floor-line">Challenger Deep · bottom of the Mariana Trench</div>
  <span class="floor-depth">10,935 m</span>`;b.appendChild(Y);const E=document.getElementById("zone-toast"),Be=E.querySelector(".zt-title"),Se=E.querySelector(".zt-sub");let se=null;function qe(e){Be.textContent=`The ${e.name} Zone`,Se.textContent=`${e.sci} · ${e.light}`,E.classList.add("show"),clearTimeout(se),se=setTimeout(()=>E.classList.remove("show"),2400)}const Ae=[{el:document.querySelector(".biolum-a"),speed:.05,tile:360},{el:document.querySelector(".biolum-b"),speed:.11,tile:440},{el:document.querySelector(".biolum-c"),speed:.19,tile:600}];let O=!1,H=null,ae=-1;function K(){O=!1;const e=Math.max(0,Math.min(window.scrollY,R)),t=e/z,i=window.innerHeight/z;A.style.setProperty("--sea-hi",_(t-i/2)),A.style.setProperty("--sea-lo",_(t+i/2)),A.style.setProperty("--sunlight",oe(t).toFixed(4));const a=Math.max(0,Math.min(1,(t-900)/400));if(A.style.setProperty("--biolum",a.toFixed(3)),a>0)for(const r of Ae)r.el.style.transform=`translate3d(0, ${-(e*r.speed%r.tile)}px, 0)`;const s=Math.round(t);s!==ae&&(ae=s,L.depth.textContent=s.toLocaleString("en-US"),L.light.textContent=fe(t),L.pressure.textContent=pe(t).toLocaleString("en-US"));const o=T(t);if(o!==H){const r=H===null;H=o,L.zone.textContent=o.name,L.zoneSci.textContent=o.sci,r||qe(o)}}window.addEventListener("scroll",()=>{O||(O=!0,requestAnimationFrame(K))},{passive:!0});window.addEventListener("resize",K,{passive:!0});K();Le();"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("./sw.js")});
