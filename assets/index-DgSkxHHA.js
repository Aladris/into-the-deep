(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))r(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const d of s.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&r(d)}).observe(document,{childList:!0,subtree:!0});function o(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function r(i){if(i.ep)return;i.ep=!0;const s=o(i);fetch(i.href,s)}})();const M=10935,p=6,Q=[{name:"Sunlight",sci:"Epipelagic",from:0,to:200,light:"Full sunlight"},{name:"Twilight",sci:"Mesopelagic",from:200,to:1e3,light:"Dim blue light"},{name:"Midnight",sci:"Bathypelagic",from:1e3,to:4e3,light:"Bioluminescence only"},{name:"Abyssal",sci:"Abyssopelagic",from:4e3,to:6e3,light:"Total darkness"},{name:"Hadal",sci:"Hadalpelagic",from:6e3,to:11e3,light:"Total darkness"}];function L(e){return Q.find(t=>e<t.to)??Q[Q.length-1]}const m=[[0,56,172,216],[80,24,126,184],[200,10,82,142],[450,9,50,104],[700,9,33,76],[1e3,7,18,50],[1600,5,11,34],[2500,4,7,24],[4e3,2,4,14],[6e3,1,2,8],[8500,0,1,4],[11e3,0,0,1]];function q(e){const t=Math.max(0,Math.min(M,e));let o=1;for(;o<m.length-1&&m[o][0]<t;)o++;const[r,i,s,d]=m[o-1],[E,O,F,N]=m[o],R=E===r?0:(t-r)/(E-r),u=(T,W)=>Math.round(T+(W-T)*R);return`rgb(${u(i,O)}, ${u(s,F)}, ${u(d,N)})`}const J=Math.log(100)/200;function P(e){return Math.exp(-J*Math.max(0,e))}function V(e){const t=P(e)*100;return e>=1e3?"0%":t>=1?`${Math.round(t)}%`:t>=.01?`${t.toFixed(2)}%`:t>=1e-4?`${t.toFixed(4)}%`:"<0.0001%"}function U(e){return Math.round(1+e/10)}const Y=[{id:"flying-fish",name:"Flying Fish",sci:"Exocoetidae",depth:6,range:"Surface",size:120,side:"right",fact:"To escape tuna and marlin, it rockets out of the water and glides on outstretched fins — some stay airborne for over 200 metres before splashing back down.",svg:`
      ...
      </svg>`}],G=[...Y].sort((e,t)=>e.depth-t.depth);G.forEach((e,t)=>{e.side=t%2===0?"right":"left"});const n=document.getElementById("ocean"),y=document.documentElement,f={depth:document.getElementById("hud-depth"),zone:document.getElementById("hud-zone"),zoneSci:document.getElementById("hud-zone-sci"),light:document.getElementById("hud-light"),pressure:document.getElementById("hud-pressure")},Z=M*p;n.style.height=`calc(${Z}px + 100svh)`;function z(e){return`calc(${Math.round(e*p)}px + 50svh)`}const w=document.createElement("div");w.id="ruler";w.setAttribute("aria-hidden","true");w.style.top="50svh";w.style.height=`${Z}px`;n.appendChild(w);for(let e=500;e<=M;e+=500){const t=document.createElement("span");t.className="ruler-label",t.textContent=`${e.toLocaleString("en-US")} m`,t.style.top=z(e),n.appendChild(t)}for(const e of Q.slice(1)){const t=document.createElement("div");t.className="zone-marker",t.style.top=z(e.from),t.innerHTML=`
    <div class="zone-marker-text">
      ${e.name} Zone · ${e.from.toLocaleString("en-US")} m
      <small>${e.sci}</small>
    </div>`,n.appendChild(t)}const $=new IntersectionObserver(e=>{for(const t of e)t.isIntersecting&&(t.target.classList.add("in-view"),$.unobserve(t.target))},{rootMargin:"0px 0px -12% 0px"});for(const e of G){const t=document.createElement("article");t.className=e.depth>=1e3?"creature deep":"creature",t.dataset.side=e.side,t.dataset.id=e.id,t.style.top=z(e.depth),t.innerHTML=`
    ...
    </div>`,t.querySelector(".creature-tap").addEventListener("click",()=>K(e)),n.appendChild(t),$.observe(t)}const a=document.createElement("div");a.id="detail";a.hidden=!0;a.innerHTML=`
  ...
  </div>`;document.body.appendChild(a);const l={sprite:a.querySelector(".detail-sprite"),zone:a.querySelector("#detail-zone"),zonefull:a.querySelector("#detail-zonefull"),name:a.querySelector("#detail-name"),sci:a.querySelector("#detail-sci"),range:a.querySelector("#detail-range"),fact:a.querySelector("#detail-fact"),card:a.querySelector(".detail-card")};let v=null;function K(e){const t=L(e.depth);l.sprite.innerHTML=e.svg,l.sprite.style.maxWidth=`${Math.min(e.size*1.7,260)}px`,l.zone.textContent=`${t.name} Zone`,l.zonefull.textContent=`${t.name} · ${t.sci}`,l.name.textContent=e.name,l.sci.textContent=e.sci,l.range.textContent=e.range,l.fact.textContent=e.fact,a.dataset.zone=t.name.toLowerCase(),v=document.activeElement,a.hidden=!1,document.body.style.overflow="hidden",requestAnimationFrame(()=>a.classList.add("open")),a.querySelector(".detail-close").focus()}function B(){a.classList.remove("open"),document.body.style.overflow="";const e=()=>{a.hidden=!0,a.removeEventListener("transitionend",e)};a.addEventListener("transitionend",e),v&&v.focus()}a.addEventListener("click",e=>{e.target.hasAttribute("data-close")&&B()});document.addEventListener("keydown",e=>{e.key==="Escape"&&!a.hidden&&B()});let g=null;l.card.addEventListener("touchstart",e=>{g=e.touches[0].clientY},{passive:!0});l.card.addEventListener("touchmove",e=>{if(g===null)return;const t=e.touches[0].clientY-g;t>0&&(l.card.style.transform=`translateY(${Math.min(t,200)}px)`)},{passive:!0});l.card.addEventListener("touchend",e=>{const t=e.changedTouches[0].clientY-(g??0);l.card.style.transform="",t>90&&B(),g=null},{passive:!0});const h=document.createElement("button");h.id="divemap-toggle";h.type="button";h.setAttribute("aria-label","Open the dive line to jump to a creature");h.innerHTML=`
  ...
  <span>DIVE<br/>LINE</span>`;document.body.appendChild(h);const c=document.createElement("div");c.id="divemap";c.hidden=!0;c.innerHTML=`
  ...
  </aside>`;document.body.appendChild(c);const I=c.querySelector(".divemap-track");let j=null;for(const e of G){const t=L(e.depth);if(t.name!==j){j=t.name;const r=document.createElement("div");r.className="divemap-zone",r.dataset.zone=t.name.toLowerCase();const i=t.to>=11e3?"10,935":t.to.toLocaleString("en-US");r.innerHTML=`<span>${t.name} · ${t.from.toLocaleString("en-US")}–${i} m</span>`,I.appendChild(r)}const o=document.createElement("button");o.type="button",o.className="divemap-item",o.dataset.side=e.side,o.setAttribute("aria-label",`Jump to ${e.name} at ${Math.round(e.depth)} metres`),o.innerHTML=`
    <span class="dm-chip">
      <span class="dm-node">${e.svg}</span>
      <span class="dm-label"><b>${e.name}</b><i>${Math.round(e.depth).toLocaleString("en-US")} m</i></span>
    </span>`,o.addEventListener("click",()=>_(e)),I.appendChild(o)}function X(){c.hidden=!1,document.body.style.overflow="hidden",requestAnimationFrame(()=>c.classList.add("open"))}function S(){c.classList.remove("open"),document.body.style.overflow="";const e=()=>{c.hidden=!0,c.removeEventListener("transitionend",e)};c.addEventListener("transitionend",e)}function _(e){S();const t=window.matchMedia("(prefers-reduced-motion: reduce)").matches;window.scrollTo({top:Math.round(e.depth*p),behavior:t?"auto":"smooth"})}h.addEventListener("click",X);c.addEventListener("click",e=>{e.target.hasAttribute("data-close")&&S()});document.addEventListener("keydown",e=>{e.key==="Escape"&&!c.hidden&&S()});const A=document.createElement("div");A.id="floor";A.innerHTML=`
  <div class="floor-line">Challenger Deep · bottom of the Mariana Trench</div>
  <span class="floor-depth">10,935 m</span>`;n.appendChild(A);const k=document.getElementById("zone-toast"),ee=k.querySelector(".zt-title"),te=k.querySelector(".zt-sub");let H=null;function ie(e){ee.textContent=`The ${e.name} Zone`,te.textContent=`${e.sci} · ${e.light}`,k.classList.add("show"),clearTimeout(H),H=setTimeout(()=>k.classList.remove("show"),2400)}const ae=[{el:document.querySelector(".biolum-a"),speed:.05,tile:360},{el:document.querySelector(".biolum-b"),speed:.11,tile:440},{el:document.querySelector(".biolum-c"),speed:.19,tile:600}];let x=!1,b=null,D=-1;function C(){x=!1;const e=Math.max(0,Math.min(window.scrollY,Z)),t=e/p,o=window.innerHeight/p;y.style.setProperty("--sea-hi",q(t-o/2)),y.style.setProperty("--sea-lo",q(t+o/2)),y.style.setProperty("--sunlight",P(t).toFixed(4));const r=Math.max(0,Math.min(1,(t-900)/400));if(y.style.setProperty("--biolum",r.toFixed(3)),r>0)for(const d of ae)d.el.style.transform=`translate3d(0, ${-(e*d.speed%d.tile)}px, 0)`;const i=Math.round(t);i!==D&&(D=i,f.depth.textContent=i.toLocaleString("en-US"),f.light.textContent=V(t),f.pressure.textContent=U(t).toLocaleString("en-US"));const s=L(t);if(s!==b){const d=b===null;b=s,f.zone.textContent=s.name,f.zoneSci.textContent=s.sci,d||ie(s)}}window.addEventListener("scroll",()=>{x||(x=!0,requestAnimationFrame(C))},{passive:!0});window.addEventListener("resize",C,{passive:!0});C();"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("./sw.js")});
